...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-500 text-sm mt-1">Unggah dokumen resmi (AD/ART, Perdes, Bukti Setor PADes) untuk dinilai
                kepatuhan BUMDesa Anda.</p>
        </div>
                ...
                <tbody>
                    @foreach ($dokumen as $dok)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td class="px-6 py-4 font-bold text-gray-900">{{ $dok->year ?? $dok->tahun }}</td>
                            <td class="px-6 py-4 font-medium">
                                {{ $dok->type ?? ($dok->tipe ?? ($dok->tipe_dokumen ?? $dok->title)) }}</td>
                            <td class="px-6 py-4 text-center">
                                @php $fileLink = $dok->file_url ?? $dok->file_dokumen ?? null; @endphp
                                @if ($fileLink)
                                    <a href="{{ asset('storage/' . $fileLink) }}" target="_blank"
                            ...
                            <td class="px-6 py-4 text-right">
                                <form action="{{ route('user.transparansi.destroy', $dok->id) }}" method="POST"
                                    class="inline-block" onsubmit="return confirm('Hapus dokumen transparansi ini?');">
                                    @csrf @method('DELETE')
                                    <button type="submit"
    ...
    <div id="modal-add" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-xl bg-white mb-20">
            ...
            <form action="{{ route('user.transparansi.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-4">
                @csrf
                <div>
                ...
                <div class="mt-4">
                    ...
                    <input type="file" name="file_dokumen" accept=".pdf" required
                        class="mt-1 block w-full border text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-gray-200 file:text-gray-700 border-gray-300 rounded-md p-2 bg-gray-50">
                ...
                <div class="pt-4 border-t flex justify-end gap-2">
                    ...
                    <button type="submit"
                        class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Unggah
                        Dokumen</button>
                </div>
            </form>
        </div>
