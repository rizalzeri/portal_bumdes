...
@section('content')
    <div class="mb-6">
        ...
        <p class="text-gray-500 text-sm mt-1">Grafik perkembangan pendapatan dan laba bersih BUMDesa Anda berdasarkan laporan
            yang diunggah.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-gradient-to-br from-blue-500 to-blue-700 text-white rounded-xl shadow-sm border p-6">
            ...
            <p class="text-3xl font-bold font-mono">
                @php
                    $latest = $bumdes->laporanKeuangan->last();
                    $latestAset = $latest ? $latest->total_aset ?? ($latest->aset ?? 0) : 0;
                @endphp
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-700 text-white rounded-xl shadow-sm border p-6">
            ...
            <p class="text-3xl font-bold font-mono">
                @php
                    $totalLaba = $bumdes->laporanKeuangan->sum(function ($lap) {
                        return $lap->laba_rugi ?? ($lap->laba_bersih ?? 0);
                    });
                @endphp
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6">
        <div class="flex justify-between items-center mb-6">
            <h3 class="font-bold text-gray-900"><i class="fa-solid fa-chart-area mr-2 text-primary"></i> Grafik Tren
                Finansial</h3>
        </div>
        ...
        @else
            <div class="text-center py-16 text-gray-400">
                ...
                <a href="{{ route('user.finansial.index') }}"
                    class="inline-block mt-4 text-primary font-bold hover:underline">Unggah Laporan Keuangan</a>
            </div>
        @endif
    </div>
