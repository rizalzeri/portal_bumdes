...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-white rounded-xl border shadow-sm p-5 flex gap-4 items-start hover:shadow-md transition">
        @if($m->logo)
            <img src="{{ Storage::url($m->logo) }}" class="w-14 h-14 object-contain border rounded-lg bg-gray-50 p-1 shrink-0" alt="{{ $m->name }}">
        @else
            <div class="w-14 h-14 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 shrink-0">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex gap-2 mt-3">
                <button onclick="openEditModal({{ $m->id }}, '{{ addslashes($m->name) }}', '{{ addslashes($m->description) }}')"
                    class="text-xs text-blue-600 font-semibold hover:underline">
                    <i class="fa-solid fa-pen mr-1"></i>Edit
                </button>
                <form action="{{ route('user.mitra.destroy', $m) }}" method="POST" onsubmit="return confirm('Hapus mitra ini?')">
                    @csrf @method('DELETE')
...
<div id="modalTambah" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <form action="{{ route('user.mitra.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            @csrf
            <div>
...
<div id="modalEdit" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        <div class="flex justify-between items-center px-6 py-4 border-b">
            ...
            <button onclick="document.getElementById('modalEdit').classList.add('hidden')" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
        </div>
        <form id="editForm" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" onclick="document.getElementById('modalEdit').classList.add('hidden')" class="px-4 py-2 rounded-lg text-sm border text-gray-600">Batal</button>
                <button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg text-sm font-semibold">Update</button>
            </div>
        </form>
    </div>
</div>
