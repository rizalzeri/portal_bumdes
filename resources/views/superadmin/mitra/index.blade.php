...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Kelola daftar instansi dan perusahaan yang menjalin kemitraan strategis.</p>
    </div>
        ...
        <table class="datatable w-full whitespace-nowrap text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3">Nama Instansi / Perusahaan</th>
                    <th class="px-6 py-3">Website & Deskripsi</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    <td class="px-6 py-4">
                        @if($mitra->logo)
                            <img src="{{ asset('storage/'.$mitra->logo) }}" class="w-16 h-16 object-contain bg-white rounded border p-1 border-gray-200 shadow-sm">
                        @else
                            <div class="w-16 h-16 bg-gray-100 flex items-center justify-center rounded border"><i class="fa-solid fa-building text-gray-400"></i></div>
                        @endif
                    </td>
                    <td class="px-6 py-4 font-bold text-gray-900 text-base whitespace-normal">{{ $mitra->name }}</td>
                    <td class="px-6 py-4 whitespace-normal">
                        @if($mitra->website)
                            <a href="{{ $mitra->website }}" target="_blank" class="text-xs inline-block bg-blue-50 text-blue-600 px-2 py-1 rounded hover:bg-blue-100 mb-2 whitespace-nowrap"><i class="fa-solid fa-globe mr-1"></i> Buka Website</a>
                        @endif
                        <p class="text-xs text-gray-500 line-clamp-2">{{ $mitra->description ?? 'Tidak ada deskripsi.' }}</p>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <button onclick="editMitra({{ $mitra->id }}, '{{ addslashes($mitra->name) }}', '{{ addslashes($mitra->description ?? '') }}', '{{ addslashes($mitra->website ?? '') }}')" class="text-accent hover:text-yellow-600 bg-yellow-50 hover:bg-yellow-100 p-2 rounded-md transition-colors mr-1">
                            <i class="fa-solid fa-pen"></i>
                        </button>
                        <form action="{{ route('superadmin.mitra.destroy', $mitra->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus mitra ini?');">
                            @csrf @method('DELETE')
...
<div id="modal-add" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-xl bg-white mb-20">
        ...
        <form action="{{ route('superadmin.mitra.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="file" name="logo" accept="image/*" class="mt-1 block w-full border text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white border-gray-300 rounded-md p-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">URL Website (Diawali https://)</label>
                <input type="url" name="website" placeholder="https://..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
            </div>
            <div>
            ...
            <div class="pt-4 border-t flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Simpan Mitra</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-edit" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-xl bg-white mb-20">
        ...
        <form id="form-edit" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Ganti Logo (Kosongkan bila tidak diganti)</label>
                <input type="file" name="logo" accept="image/*" class="mt-1 block w-full border text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white border-gray-300 rounded-md p-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">URL Website</label>
                <input type="url" name="website" id="edit-website" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Deskripsi Singkat</label>
            ...
            <div class="pt-4 border-t flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Perbarui Mitra</button>
            </div>
        </form>
    </div>
</div>
