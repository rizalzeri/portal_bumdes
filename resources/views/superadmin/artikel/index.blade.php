        ...
        <table class="datatable w-full whitespace-nowrap text-sm text-left text-gray-500">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    <td class="px-6 py-4">
                        @if($art->image)
                            <img src="{{ asset('storage/'.$art->image) }}" class="w-16 h-12 object-cover rounded shadow-sm border">
                        @else
                            <div class="w-16 h-12 bg-gray-100 flex items-center justify-center rounded border"><i class="fa-solid fa-image text-gray-400"></i></div>
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-normal">
                        <div class="bg-blue-100 text-blue-800 text-[10px] font-bold px-2 py-0.5 rounded uppercase inline-block mb-1">{{ $art->category }}</div>
                        <div class="font-bold text-gray-900 leading-tight">{{ $art->title }}</div>
                    </td>
                    <td class="px-6 py-4">{{ $art->created_at->format('d/m/Y') }}</td>
                    <td class="px-6 py-4 text-right">
                        <button onclick="editArtikel({{ $art->id }}, '{{ addslashes($art->title) }}', '{{ addslashes($art->category) }}', '{{ addslashes($art->content) }}')" class="text-accent hover:text-yellow-600 bg-yellow-50 hover:bg-yellow-100 p-2 rounded-md transition-colors mr-1">
                            <i class="fa-solid fa-pen"></i>
                        </button>
                        <form action="{{ route('superadmin.artikel.destroy', $art->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus artikel ini?');">
                            @csrf @method('DELETE')
...
<div id="modal-add" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-3xl shadow-lg rounded-xl bg-white mb-20">
        ...
        <form action="{{ route('superadmin.artikel.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <select name="category" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
            ...
            <div>
                ...
                <input type="file" name="image" accept="image/*" class="mt-1 block w-full border text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-primary-900 p-2 rounded-md">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Isi Konten</label>
                <textarea name="content" rows="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2"></textarea>
            </div>
            <div class="pt-4 border-t flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Terbitkan</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="relative top-10 mx-auto p-5 border w-full max-w-3xl shadow-lg rounded-xl bg-white mb-20">
        ...
        <form id="form-edit" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="edit-title" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Kategori</label>
                <select name="category" id="edit-category" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
                    <option value="Berita">Berita</option>
                    <option value="Cerita Sukses">Cerita Sukses</option>
                    <option value="Opini Pakar">Opini Pakar</option>
                    <option value="Regulasi">Regulasi</option>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Ganti Gambar (Kosongkan jika tak ingin ganti)</label>
                <input type="file" name="image" accept="image/*" class="mt-1 block w-full border text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white p-2 rounded-md">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Isi Konten</label>
                <textarea name="content" id="edit-content" rows="10" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2"></textarea>
            </div>
            <div class="pt-4 border-t flex justify-end gap-2">
