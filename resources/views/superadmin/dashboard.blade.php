...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border p-6">
        <div class="flex justify-between items-center mb-6 border-b pb-4">
            <h3 class="text-lg font-bold text-gray-800"><i class="fa-solid fa-chart-area text-primary mr-2"></i> Grafik Pendaftaran BUMDesa (Dummy)</h3>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6">
        <h3 class="text-lg font-bold text-gray-800 mb-6 border-b pb-4"><i class="fa-solid fa-bolt text-accent mr-2"></i> Pendaftaran BUMDesa Terbaru</h3>
        ...
        @else
            <div class="space-y-4">
                ...
                <div class="flex items-start">
                    <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center border font-bold text-primary flex-shrink-0">
                        {{ substr($bd->name, 0, 1) }}
                    </div>
                    <div class="ml-3">
                        <p class="text-sm font-bold text-gray-900 border-b-2 border-transparent hover:border-accent inline-block cursor-pointer transition">{{ $bd->name }}</p>
                        <p class="text-xs text-gray-500">{{ $bd->kabupaten->name ?? '-' }}, {{ $bd->kabupaten->province->name ?? '-' }}</p>
                        <p class="text-[10px] text-gray-400 mt-1"><i class="fa-regular fa-clock"></i> {{ $bd->created_at->diffForHumans() }}</p>
                    </div>
                </div>
                @endforeach
            </div>
            <div class="mt-6 border-t pt-4">
                <a href="{{ route('superadmin.user.index') }}" class="text-sm font-medium text-primary hover:text-accent w-full text-center block">Lihat Semua Pengguna <i class="fa-solid fa-arrow-right text-xs ml-1"></i></a>
            </div>
        @endif
    </div>
</div>
