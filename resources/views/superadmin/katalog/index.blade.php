...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @foreach ($katalogs as $kt)
            <div class="bg-white border rounded-xl overflow-hidden hover:shadow-lg transition-shadow flex flex-col relative">
                <div class="h-48 bg-gray-200 relative">
                    ...
                    @else
                        <div class="w-full h-full flex justify-center items-center"><i
                                class="fa-solid fa-image text-gray-400 text-5xl"></i></div>
                    @endif

                    <div class="absolute top-2 left-2 flex flex-col gap-1">
                        @if ($kt->category)
                            <div
                                class="bg-blue-600 text-white text-[10px] uppercase font-bold px-2 py-1 rounded shadow-sm w-fit">
                                {{ $kt->category }}</div>
                        @endif
                        @if ($kt->bumdes_id)
                            <div
                                class="bg-white text-gray-800 text-[9px] font-bold px-2 py-1 rounded shadow-sm border border-gray-100 w-fit">
                                <i class="fa-solid fa-store mr-1 text-primary"></i>{{ $kt->bumdes->name }}
                            </div>
                        @else
                            <div
                                class="bg-blue-100 text-blue-700 text-[9px] font-bold px-2 py-1 rounded shadow-sm border border-blue-200 w-fit uppercase">
                                Portal Pusat</div>
                        @endif
                    </div>

                    <div class="absolute top-2 right-2 flex flex-col gap-1.5">
                        <form action="{{ route('superadmin.katalog.toggle_featured', $kt->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="transition-all p-2 rounded-lg shadow-md {{ $kt->is_featured ? 'bg-yellow-400 text-white' : 'bg-white/90 text-gray-400' }}"
                                title="{{ $kt->is_featured ? 'Hapus dari Unggulan' : 'Set Jadi Unggulan' }}">
                                <i class="fa-solid fa-star text-sm"></i>
                            </button>
                        </form>
                        <div class="flex flex-col gap-1">
                            <button onclick="editKatalog({{ $kt->toJson() }})"
                                class="bg-white/90 hover:bg-white text-gray-600 p-2 rounded-lg transition shadow-md"><i
                                    class="fa-solid fa-pen text-xs"></i></button>
                            <form action="{{ route('superadmin.katalog.destroy', $kt->id) }}" method="POST"
                                onsubmit="return confirm('Hapus produk ini?');" class="inline">
                                @csrf @method('DELETE')
                                <button
                                    class="bg-white/90 hover:bg-red-50 text-red-500 p-2 rounded-lg transition shadow-md"><i
                    ...
                    <div class="mt-4 pt-4 border-t flex justify-between items-end">
                        <div>
                            ...
                            <span class="font-bold text-green-600">Rp
                                {{ number_format($kt->price ?? 0, 0, ',', '.') }}</span>
                        </div>
                        @if ($kt->link_pembelian)
                            <a href="{{ $kt->link_pembelian }}" target="_blank"
                                class="text-xs bg-primary text-white px-2 py-1 rounded hover:bg-primary-900 transition"><i
                                    class="fa-solid fa-cart-shopping mr-1"></i> Beli</a>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('superadmin.katalog.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="category" placeholder="Cth: Kerajinan"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
                    </div>
                    <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Link Pembelian Eksternal
                        (Tokopedia/Shopee/WA)</label>
                    <input type="url" name="link_pembelian" placeholder="https://"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
                </div>
                <div>
        ...
        <div class="relative top-10 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-xl bg-white mb-10">
            ...
            <form id="form-edit" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="url" name="link_pembelian" id="edit-link"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Deskripsi</label>
