...
<div class="mb-6 flex justify-between items-center">
    <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Unggah file dokumen PDF, Word, atau Excel untuk dapat diunduh publik dan BUMDesa.</p>
    </div>
        ...
        <table class="datatable w-full whitespace-nowrap text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3">Kategori Jenis</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    <td class="px-6 py-4 font-bold text-gray-900 whitespace-normal">
                        <i class="fa-solid fa-file-lines text-red-500 mr-2"></i> {{ $mat->title }}
                    </td>
                    <td class="px-6 py-4">
                        <span class="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded border border-blue-200 capitalize">
                            {{ $mat->type }}
                        </span>
                    </td>
                    <td class="px-6 py-4">{{ $mat->created_at->format('d/m/Y H:i') }}</td>
                    <td class="px-6 py-4 text-right">
                        <a href="{{ asset('storage/'.$mat->file_url) }}" target="_blank" class="text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 p-2 rounded-md transition-colors mr-1 inline-block" title="Download">
                            <i class="fa-solid fa-download"></i>
                        </a>
                        <form action="{{ route('superadmin.materi.destroy', $mat->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus dokumen ini dari server?');">
                            @csrf @method('DELETE')
...
<div id="modal-add" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-xl bg-white">
        ...
        <form action="{{ route('superadmin.materi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700">Judul Dokumen</label>
                <input type="text" name="title" required placeholder="Cth: Panduan Pendaftaran BUMDesa" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="file" name="file_url" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip" required class="mt-1 block w-full rounded-md border border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm p-2 bg-gray-50 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-white hover:file:bg-primary-900 cursor-pointer">
            ...
            <div class="pt-4 border-t flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Upload Data</button>
            </div>
        </form>
    </div>
</div>
