...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Pengumuman Internal Kabupaten</h2>
        <p class="text-gray-500 text-sm mt-1">Buat pengumuman atau edaran khusus untuk seluruh BUMDesa di wilayah Kabupaten Anda.</p>
    </div>
        ...
        <table class="datatable w-full whitespace-nowrap text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3" style="min-width: 300px">Isi / Konten (Preview)</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 font-bold text-gray-900 whitespace-normal text-base">{{ $p->title }}</td>
                    <td class="px-6 py-4 whitespace-normal text-xs text-gray-500 line-clamp-2">
                        {{ Str::limit(strip_tags($p->content), 100) }}
                    </td>
                    <td class="px-6 py-4 text-right">
                        <button onclick="editPengumuman({{ $p->id }}, '{{ addslashes($p->title) }}', '{{ addslashes($p->content) }}')" class="text-accent hover:text-yellow-600 bg-yellow-50 hover:bg-yellow-100 p-2 rounded-md transition-colors mr-1">
                            <i class="fa-solid fa-pen"></i>
                        </button>
                        <form action="{{ route('adminkab.pengumuman.destroy', $p->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus pengumuman ini secara permanen?');">
                            @csrf @method('DELETE')
...
<div id="modal-add" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-xl bg-white mb-20">
        <div class="flex justify-between items-center mb-4 border-b pb-2">
            <h3 class="text-xl font-bold text-gray-900">Buat Pengumuman Internal</h3>
        ...
        <form action="{{ route('adminkab.pengumuman.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700">Judul Pengumuman <span class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Isi Pengumuman <span class="text-red-500">*</span></label>
                <textarea name="content" rows="6" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2"></textarea>
                <p class="text-xs text-gray-500 mt-1">Pengumuman ini HANYA akan tayang pada dashboard BUMDesa yang terdaftar di Kabupaten Anda.</p>
            </div>
            <div class="pt-4 border-t flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-900">Publikasikan ke BUMDesa</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-edit" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-xl bg-white mb-20">
        <div class="flex justify-between items-center mb-4 border-b pb-2">
            <h3 class="text-xl font-bold text-gray-900">Edit Pengumuman Internal</h3>
        ...
        <form id="form-edit" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="content" id="edit-content" rows="6" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border p-2"></textarea>
            </div>
            <div class="pt-4 border-t flex justify-end gap-2">
...
<script>
    function editPengumuman(id, title, content) {
        document.getElementById('form-edit').action = `/adminkab/pengumuman/${id}`;
